...
class UnstructuredChunker(Chunker):
    def chunk(self, book_path: str) -> list[Chunk]:
        ...
        for doc in loader.lazy_load():
            if len(doc.page_content) == 0 or doc.metadata["category"] != "NarrativeText":
                continue
        ...
        chunks: list[Chunk] = []
        embeddings = self.get_embeddings(texts=parsed_text)
    ...
    def chunk(self, book_path: str, toc: TableOfContents, text_initial_page: int = None) -> list[Chunk]:
        ...
        for index, chapter in enumerate(tqdm(toc.chapters)):
            last_page = toc.chapters[index + 1].start_page - 1 if index + 1 < len(toc.chapters) else page_count
            loader = self._load_pages(book_path=book_path, start_page=chapter.start_page, end_page=last_page)
    ...
    def get_embeddings(texts: list[str], batch_size: int = 16) -> list[list[float]]:
        ...
        for index in range(num_batches):
            batch = texts[index * batch_size : (index + 1) * batch_size]
            ...
            if not response.data:
                raise ValueError(f"Failed to generate embeddings for batch {index}")

            for batch_index, embedding_data in enumerate(response.data):
                if embedding_data.embedding is None:
                    raise ValueError(
                        f"Failed to generate embedding for text at index {index * batch_size + batch_index}"
                    )
